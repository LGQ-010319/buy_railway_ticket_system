 ...
 */
public class HelpBuyServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public HelpBuyServlet() {
        super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		List<Train> trainList = trainService.getAll();//调用TrainServiceImpl()里的getAll()
		if(trainList != null) {
		...
		request.getRequestDispatcher("pages/admin/right/helpBuy.jsp").forward(request, response);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(list!=null && list.size()>0) {
			...
			String path = "<script>alert('很抱歉，没有查询到车次！');"
				    + "window.location.href='"+basePath+"HelpBuyServlet'</script>";
